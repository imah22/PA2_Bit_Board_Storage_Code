...
private:
   ...
   /**
    * Checks if the given row and column is valid for this bit array
    * @param row - row to access
    * @param column - column to access
    * @throws BitArray2DException in case the indices are not valid
    */
   void check_array_bounds(unsigned int row, unsigned int column) const;
